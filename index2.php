...
<?php foreach ($_SESSION['carros'] as $carro): ?>
  <div class="card">
    <img src="<?= htmlspecialchars($carro['imagem']) ?>" alt="<?= htmlspecialchars($carro['nome']) ?>" />
    <h2><?= htmlspecialchars($carro['nome']) ?></h2>
    <p>R$ <?= number_format($carro['preco'], 2, ',', '.') ?>/dia</p>
    <button class="btn-info" onclick="irParaDetalhe('<?= htmlspecialchars($carro['nome']) ?>')">Alugar</button>
  </div>
<?php endforeach; ?>
